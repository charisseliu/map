...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        ...
        rest_list.append([cluster, restaurant]) 
    return group_by_first(rest_list)
    # END Question 4
...
def k_means(restaurants, k, max_updates=100):
    ...
    while previous_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        groups = group_by_centroid(restaurants, previous_centroids)
        centroids = [find_centroid(restaurants) for restaurants in groups]
        # END Question 6
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    maxf = max(feature_fns, key= lambda feature: (find_predictor(user, reviewed, feature)[1]))
    return find_predictor(user, reviewed, maxf)[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    d = {restaurant_name(restaurant): 0 for restaurant in restaurants}
    for restaurant in restaurants:
        if restaurant in reviewed:
            d[restaurant_name(restaurant)] = user_score(user, restaurant_name(restaurant))
        else:
